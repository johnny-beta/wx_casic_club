...
BmobSocketIo.initialize('0bfe14d135afa2dd2abf54bb979ef145');
var app = getApp();
...
Page({
  ...
  enter() {
    // 如果登录过，会记录当前用户openid
    // debugger
    var newOpenid = app.globalData.currentUser.openid
    if (!newOpenid) {
  ...
  connect() {
    ...
    BmobSocketIo.onUpdateTable = function(tablename, data) {
      console.log('触发onUpdateTable')
      if (tablename == "Chat") {
        that.pushMessage(createUserMessage(data.content, data));
      }
  ...
  sendMessage(e) {
    ...
    //写入用户信息方便判断谁发送的消息
    diary.set("nickName", currentUser["nickName"]);
    diary.set("avatarUrl", currentUser["userPic"]);
...
function welcome(that) {
 ...
 // UserModel.id = objectId;
  diary.set("openid", currentUser['openid']);
  diary.set("online", 0);
  diary.set("nickName", currentUser["nickName"]);
  //添加数据，第一个入口参数是null
  diary.save().then(result =>{
